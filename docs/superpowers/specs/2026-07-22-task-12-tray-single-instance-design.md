...
完整技术方案中列出的日志目录、重启扫描组件、重启本机服务和开机启动菜单不在本 Task 实现；其中开机启动由后续安装包 Task 处理。
...
`TrayApplication` 负责组装单实例守卫、Uvicorn Server 和 `pystray.Icon`，并允许测试注入 Server、Icon、互斥体和目录打开器。
...
4. 托盘状态菜单项从当前 ApplicationContext/Worker 状态读取就绪状态、PID 和 generation。
